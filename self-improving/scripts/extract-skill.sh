...
# Validate
case "$CATEGORY" in common|work|personal) ;; *) echo -e "${RED}Invalid category: $CATEGORY${NC}"; usage ;; esac
[[ "$SKILL_NAME" =~ ^[a-z0-9]+(-[a-z0-9]+)*$ ]] || { echo -e "${RED}Invalid name. Use lowercase + hyphens.${NC}"; exit 1; }

SKILL_PATH="$SKILLS_ROOT/$CATEGORY/$SKILL_NAME"
[ -d "$SKILL_PATH" ] && { echo -e "${RED}Already exists: $SKILL_PATH${NC}"; exit 1; }
...
if $DRY_RUN; then
  echo -e "${YELLOW}[DRY RUN]${NC} Would create:"
  echo "  $SKILL_PATH/SKILL.md"
  echo ""
  echo "$CONTENT"
else
  mkdir -p "$SKILL_PATH"
  echo "$CONTENT" > "$SKILL_PATH/SKILL.md"
  echo -e "${GREEN}✅ Created${NC} $SKILL_PATH/SKILL.md"
  echo ""
  echo "Next: edit SKILL.md, fill TODOs, run quality check."
fi
